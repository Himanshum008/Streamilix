...
function CreatePlaylists() {
  ...
  const handleCreatePlaylist = async () => {
    if (selectedVideos.length === 0) {
      showCustomAlert("Please select at least 1 video")
    }
    setLoading(true)
    try {
    ...
    <div className='w-full min-h-[80vh] bg-[#0f0f0f] text-white flex flex-col pt-5'>
      <main className='flex flex-1 justify-center items-center px-4 py-6'>
        <div className='bg-[#121212] p-6 rounded-xl w-full max-w-2xl shadow-lg space-y-6'>
          ...
          <div>
            <p className='mb-3 text-lg font-semibold'>Selected Videos</p>
          ...
          <button disabled={!title || !description || loading} className='w-full bg-orange-600 hover:bg-orange-700 py-3 
          rounded-lg font-medium disabled:bg-gray-600 flex items-center justify-center' onClick={handleCreatePlaylist}>
            {loading ? <ClipLoader size={20} color='black'/> : "Create Playlist"}</button>
        </div>
      </main>
    </div>
  )
