    ...
    <div className='bg-[#0f0f0f] text-white min-h-screen flex flex-col'>
        ...
        border-gray-800 bg-[#0f0f0f] shadow-md'>
            ...
            <div className='ml-auto flex items-center gap-3 sm:gap-4'>
                <button className='bg-[#272727] px-3 sm:px-4 py-1 lg:rounded-lg rounded-full hover:bg-[#161414]
                active:scale-95 transition cursor-pointer text-sm hidden md:flex items-center justify-center gap-1
                ' onClick={()=>navigate("/create")}><FaPlusCircle /> <span>Create</span></button>
                <img src={channelData?.avatar} alt="" className='w-8 h-8 sm:w-9 sm:h-9 rounded-full border 
                border-gray-600 object-cover hover:scale-105 transiton' onClick={()=>setOpen(!open)}/>
            </div>
        </header>

        <div className='flex flex-1 flex-col md:flex-row'>
            <aside className='hidden md:flex w-56 lg:w-64 bg-[#121212] border-r 
            border-gray-800 flex-col p-4 shadow-lg'>

                <div className='flex flex-col items-center gap-2 mb-8 text-center'>
                    <img src={channelData?.avatar} alt="" className='w-28 h-28 rounded-full border border-gray-700 object-cover
        ...
        <main className=' flex-1 p-4 sm:p-6 pb-20 mb:pb-6'>
            <div className='border border-gray-700 rounded-lg p-4 sm:p-6 text-center text-gray-400bg-[#181818]
            shadow-inner min-h-[70vh]'>
                {open && <Profile/>}
                <div className='mt-4'>
                    <Outlet/>
                </div>
        ...
        border-gray-800 flex justify-around py-2 z-50'>
            <MobileNavItem
              icon={<FaTachometerAlt />}
              text="Dashboard"
...
function SidebarItem ({icon, text, onClick, active, setActive}) {
  ...
  return(
    ...
    className={`flex items-center gap-2 lg:gap-3 w-full px-3 py-2 rounded-lg transition-all
      ${isActive ? "bg-[#272727] text-white shadow-md" : 
...
function MobileNavItem ({icon, text, onClick, active}) {
  return (
    ...
    px-2 sm:px-3 py-2 rounded-lg transition-all duration-300 ${active ? "text-orange-100" : "text-gray-400" } hover:text-white
    hover:bg-[#272727]`}
    onClick={onClick}>
