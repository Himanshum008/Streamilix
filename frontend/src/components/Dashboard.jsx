...
function Dashboard() {
    ...
    const totalShortViews = (channelData.shorts || []).reduce(
        (acc, short) => acc + (short.views || 0), 0
    );
    const totalViews = totalVideoViews + totalShortViews;

  return (
    <div className='w-full text-white min-h-screen p-4 sm:p-6 space-y-6 mb-12.5'>
        <div className='flex flex-col md:flex-row items-center md:items-start gap-4'>
            <img src={channelData?.avatar} alt="" className='w-16 h-16 rounded-full object-cover border border-gray-700'/>
        ...
        <div>
            <h3 className='pl-1 text-start sm:text-lg font-semibold'>
                Channel Analytics
            </h3>
...
function ContentCard({ content, onClick }) {
  return (
    <div
      className="flex flex-col sm:flex-row gap-4 items-start bg-[#0f0f0f] border border-gray-700 
      rounded-lg p-3 sm:p-4 hover:bg-[#202020] transition"
      onClick={onClick}
    >
      {/* Thumbnail */}
      <img
